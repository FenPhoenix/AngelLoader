...
internal sealed class DeflateStreamFast : Stream
{
    internal const int DefaultBufferSize = 8192;
    private readonly Stream _stream;
    private readonly InflaterZlib inflater;
    private readonly byte[] _buffer;

    public DeflateStreamFast(Stream stream, byte[] buffer)
    {
        _stream = stream;
        if (!_stream.CanRead) throw new ArgumentException("NotReadableStream", nameof(stream));
        inflater = new InflaterZlib(-15);
        _buffer = buffer;
    }
    ...
    public override int Read(byte[] array, int offset, int count)
    {
        ...
        while (true)
        {
            int num = inflater.Inflate(array, offset1, length1);
            offset1 += num;
            length1 -= num;
            if (length1 != 0 && !inflater.Finished())
            {
                int length2 = _stream.Read(_buffer, 0, _buffer.Length);
                if (length2 != 0)
                {
                    inflater.SetInput(_buffer, 0, length2);
                }
                else
                {
                    break;
