...
public sealed partial class Scanner : IDisposable
{
    ...
    private List<string>? _solidZipExtractedFilesList;
    private List<string> SolidZipExtractedFilesList => _solidZipExtractedFilesList ??= new List<string>(50);

    private List<string>? _solidZipExtractedFilesTempList;
    private List<string> SolidZipExtractedFilesTempList => _solidZipExtractedFilesTempList ??= new List<string>(50);
    ...
    private void ResetCachedFields()
    {
        ...
        _solidZipExtractedFilesList?.Clear();
        _solidZipExtractedFilesTempList?.Clear();
        ...
        if (_fmFormat is FMFormat.SevenZip or FMFormat.RarSolid)
        {
            ...
            */
            ...
            try
            {
                ...
                var fileNamesList = SolidZipExtractedFilesList;
    ...
    string GetValueFromLines(SpecialLogic specialLogic, string[] keys, ListFast<string> lines)
    {
        for (int lineIndex = 0; lineIndex < lines.Count; lineIndex++)
        {
            ...
            if (lineStartsWithKeyAndSeparatorChar)
            {
                if (specialLogic == SpecialLogic.ReleaseDate)
                {
                    // @NET5: These are fine because they only run on detected release date lines.
                    // Turns out we can't actually remove them anyway, they need to be deduplicated for the
                    // string to date converter, it looks like. Probably. Anyway whatever, it's fine.
