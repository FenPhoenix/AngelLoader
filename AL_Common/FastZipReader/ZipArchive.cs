...
// @vNext(Common/ZipArchiveFast):
// This is the pre-perf-updates version that we know works. We should carefully vet which updates we want to apply
// and apply them before next release. The -1 to 0 check, the SplitSpanned to throw, etc.
public sealed class ZipArchiveFast : IDisposable
{
    ...
    private bool IsOpenable(ZipArchiveEntry entry, out string message)
    {
        ...
        if (entry.OffsetOfLocalHeader > _archiveStream.Length)
        {
        ...
        if (!ZipLocalFileHeader.TrySkipBlock(_archiveStream, _bundle))
        {
            message = SR.LocalFileHeaderCorrupt;
            return false;
        }
        ...
        if (entry.StoredOffsetOfCompressedData + entry.CompressedLength > _archiveStream.Length)
        {
            message = SR.LocalFileHeaderCorrupt;
            return false;
        }
