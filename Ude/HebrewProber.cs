...
internal sealed class HebrewProber : CharsetProber
{
    ...
    // @Ude: Rewrite this in a way to make the null! unnecessary.
    private CharsetProber _logicalProber = null!;
    private CharsetProber _visualProber = null!;
    private int _finalCharLogicalScore;
    private int _finalCharVisualScore;
    ...
    */
    internal override ProbingState HandleData(byte[] buf, int offset, int len, MemoryStreamFast? memoryStream)
    {
        ...
        for (int i = offset; i < max; i++)
        {
            ...
            else
            {
                ...
                if (_beforePrev == 0x20 && IsFinal(_prev) && b != ' ')
                {
                    ++_finalCharVisualScore;
                }
            }
    ...
    internal override Charset GetCharsetName()
    {
        ...
        if (finalsub <= -MIN_FINAL_CHAR_DISTANCE)
        {
            return Charset.ISO_8859_8;
        }

        // It's not dominant enough, try to rely on the model scores instead.
        float modelsub = _logicalProber.GetConfidence() - _visualProber.GetConfidence();
    ...
    internal override ProbingState GetState()
    {
        ...
        if (_logicalProber.GetState() == ProbingState.NotMe &&
            _visualProber.GetState() == ProbingState.NotMe)
        {
            return ProbingState.NotMe;
        }
    ...
    {
        return b is FINAL_KAF or FINAL_MEM or FINAL_NUN or FINAL_PE or FINAL_TSADI;
    }
    ...
    private static bool IsNonFinal(byte b)
    {
        ...
        // are quite rare.
        return b is NORMAL_KAF or NORMAL_MEM or NORMAL_NUN or NORMAL_PE;
    }
}
