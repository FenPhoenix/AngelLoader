...
internal static partial class Ini
{
    ...
    private static void Config_SettingsIOThreadingVScrollPos_Set(ConfigData config, string valTrimmed, string valRaw, GameIndex gameIndex, bool ignoreGameIndex)
    {
        if (Int_TryParseInv(valTrimmed, out int result))
        {
    ...
    private static void Config_DriveMultithreadingLevel_Set(ConfigData config, string valTrimmed, string valRaw, GameIndex gameIndex, bool ignoreGameIndex)
    {
        ...
        {
            string[] letterAndThreadability = value.Split(CA_Colon, StringSplitOptions.RemoveEmptyEntries);
            if (letterAndThreadability.Length != 2) continue;

            string letter = letterAndThreadability[0].Trim();
            if (letter.Length > 0)
            {
    ...
    {
        config.ShowWineTricksMessageIfApplicable = valTrimmed.EqualsTrue();
    }
    ...
    CreateConfigDictionary() => new(CachedKeyComparer)
    {
        ...
        { "SettingsIOThreadingVScrollPos", new Config_DelegatePointerWrapper(&Config_SettingsIOThreadingVScrollPos_Set) },
        ...
        { "ShowWineTricksMessageIfApplicable", new Config_DelegatePointerWrapper(&Config_ShowWineTricksMessageIfApplicable_Set) },
