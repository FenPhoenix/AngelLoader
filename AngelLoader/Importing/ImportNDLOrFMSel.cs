...
{
    internal static class ImportNDLOrFMSel
    {
        internal static async Task<bool>
        Import(ImportType importType, string iniFile, FieldsToImport? fields = null)
        {
            bool fmSel = importType == ImportType.FMSel;

            Core.View.ShowProgressBox(fmSel ? ProgressTasks.ImportFromFMSel : ProgressTasks.ImportFromNDL);
            try
            {
                var (error, fmsToScan) = await (fmSel
