...
internal static partial class DetectDriveData
{
    ...
    GetDriveThreadability(string root)
    {
        ...
        */
        ...
        {
            return !root.IsEmpty() && root.Length == 2 && char.IsAsciiLetter(root[0]) && root[1] == ':';
        }
    }
    ...
    private static bool PathStartsWithLetterAndVolumeSeparator(string path) =>
        path.Length >= 2 && char.IsAsciiLetter(path[0]) && path[1] == Path.VolumeSeparatorChar;
    ...
    private static (string Path, bool IsLink) GetRealDirectory(string path, IOPathType ioPathType)
    {
        try
        {
            ...
            FileSystemInfo? fsi = Directory.ResolveLinkTarget(path, returnFinalTarget: true);
            if (fsi != null)
            {
                path = fsi.FullName;
            }
