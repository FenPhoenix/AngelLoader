                ...
                if (backupFile.DarkLoader)
                {
                    for (int i = 0; i < entriesCount; i++)
                    {
                        ...
                        if (!fn.Rel_ContainsDirSep())
                        {
                            ...
                            entry.ExtractToFile_Fast(finalFilePath, overwrite: true);
                        }
                ...
                else
                {
                    ...
                    if (restoreSavesAndScreensOnly)
                    {
                        for (int i = 0; i < entriesCount; i++)
                        {
                                  ...
                                  (SS2SaveDirsInZipRegex().IsMatch(fn) || fn.PathStartsWithI(_ss2CurrentDirS)))))
                            {
                                ...
                                Directory.CreateDirectory(Path.Combine(fmInstalledPath, fn.Substring(0, fn.Rel_LastIndexOfDirSep())));
                                entry.ExtractToFile_Fast(finalFileName, overwrite: true);
                            }
                    ...
                    else
                    {
                        ...
                        if (fmSelInf != null)
                        {
                            ...
                            {
                                bool startsWithRemoveFile = line.StartsWithO(_removeFileEq);
