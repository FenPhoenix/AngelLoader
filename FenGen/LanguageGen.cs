...
namespace FenGen
{
    internal static class Language
    {
        ...
        Generate(string sourceFile, string destFile, string langIniFile, string testLangIniFile = "", bool writeReflectionStyle = false)
        {
            ...
            WriteDest(langClassName, sections, classNames, destFile, langIniFile, testLangIniFile, perGameSets, writeReflectionStyle);
        }
            ...
            bool writeReflectionStyle)
        {
            ...
            {
                w.WL("#region Autogenerated per-game string getters");
                w.WL();

                int count = Cache.GamesEnum.GameIndexEnumNames.Count;
                foreach (var gameSet in perGameSets)
                {
                    w.WL("internal static string " + gameSet.Key + "(GameSupport.GameIndex gameIndex)");
                    w.WL("{");
                    w.WL("return");
                    w.IncrementIndent();
                    ...
                    for (int i = 0; i < count; i++)
                    {
                        ...
                        string prefix = i < count - 1
                            ? "gameIndex == GameSupport.GameIndex." + Cache.GamesEnum.GameIndexEnumNames[i] +
                              " ? "
                            : "";
