    ...
    private static void WriteReadSection(CodeWriters.IndentingWriter w, string obj, List<Field> fields)
    {
        ...
        for (int i = 0; i < fields.Count; i++)
        {
            ...
            else if (field.Type == "CustomResources")
            {
                ...
                w.WL("SetFMCustomResources(" + obj + ", " + val + ");");
            }
    ...
    private static void WriteWriter(CodeWriters.IndentingWriter w, string obj, List<Field> fields)
    {
        ...
        foreach (Field field in fields)
        {
            ...
            else if (field.Type == "DateTime?")
            {
                ...
                string seconds = !field.DoNotConvertDateTimeToLocal
                    ? "new DateTimeOffset((DateTime)" + objDotField + ").ToUnixTimeSeconds();"
                    : "new DateTimeOffset(((DateTime)" + objDotField + ").ToLocalTime()).ToUnixTimeSeconds();";
            ...
            else if (field.Type == "CustomResources")
            {
                ...
                w.WL("{");
                w.WL("sw.WriteLine(\"HasMap=\" + " + obj + ".HasResource(CustomResources.Map).ToString());");
                w.WL("sw.WriteLine(\"HasAutomap=\" + " + obj + ".HasResource(CustomResources.Automap).ToString());");
                w.WL("sw.WriteLine(\"HasScripts=\" + " + obj + ".HasResource(CustomResources.Scripts).ToString());");
                w.WL("sw.WriteLine(\"HasTextures=\" + " + obj + ".HasResource(CustomResources.Textures).ToString());");
                w.WL("sw.WriteLine(\"HasSounds=\" + " + obj + ".HasResource(CustomResources.Sounds).ToString());");
                w.WL("sw.WriteLine(\"HasObjects=\" + " + obj + ".HasResource(CustomResources.Objects).ToString());");
                w.WL("sw.WriteLine(\"HasCreatures=\" + " + obj + ".HasResource(CustomResources.Creatures).ToString());");
                w.WL("sw.WriteLine(\"HasMotions=\" + " + obj + ".HasResource(CustomResources.Motions).ToString());");
                w.WL("sw.WriteLine(\"HasMovies=\" + " + obj + ".HasResource(CustomResources.Movies).ToString());");
                w.WL("sw.WriteLine(\"HasSubtitles=\" + " + obj + ".HasResource(CustomResources.Subtitles).ToString());");
                w.WL("}");
                w.WL("#else");
                w.WL("sw.Write(\"" + fieldIniName + "=\");");
                w.WL("if (" + obj + ".ResourcesScanned)");
